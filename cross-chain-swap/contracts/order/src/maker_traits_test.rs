...
use crate::{
    maker_traits::{MakerTraitsBuilder, MakerTraitsLib},
};
...
fn test_has_extension_flag() {
    ...
    assert!(MakerTraitsLib::has_extension(env.clone(), traits_with_extension));
    
    // Test without extension flag
    let traits_without_extension = MakerTraitsBuilder::new(env.clone())
        .build();
    
    assert!(!MakerTraitsLib::has_extension(env.clone(), traits_without_extension));
}
...
fn test_is_allowed_sender() {
    ...
    assert!(MakerTraitsLib::is_allowed_sender(&env, traits_any_sender, sender.clone()));
...
fn test_allow_partial_fills() {
    ...
    assert!(!MakerTraitsLib::allow_partial_fills(&env, traits_no_partial));
}
...
fn test_need_pre_interaction_call() {
    ...
    assert!(!MakerTraitsLib::need_pre_interaction_call(&env, traits_default));
    ...
    assert!(MakerTraitsLib::need_pre_interaction_call(&env, traits_pre_interaction));
}
...
fn test_need_post_interaction_call() {
    ...
    assert!(!MakerTraitsLib::need_post_interaction_call(&env, traits_default));
    ...
    assert!(MakerTraitsLib::need_post_interaction_call(&env, traits_post_interaction));
}
...
fn test_allow_multiple_fills() {
    ...
    assert!(MakerTraitsLib::allow_multiple_fills(&env, traits_multiple_fills));
}
...
fn test_use_bit_invalidator() {
    ...
    assert!(!MakerTraitsLib::use_bit_invalidator(&env, traits_both_fills));
    ...
    assert!(MakerTraitsLib::use_bit_invalidator(&env, traits_no_partial_multi));
}
...
fn test_need_check_epoch_manager() {
    ...
    assert!(!MakerTraitsLib::need_check_epoch_manager(&env, traits_default));
    ...
    assert!(MakerTraitsLib::need_check_epoch_manager(&env, traits_epoch_check));
}
...
fn test_use_permit2() {
    ...
    let traits_permit2 = MakerTraitsBuilder::new(env.clone())
        .use_permit2()
        .build();
...
fn test_unwrap_weth() {
    ...
    let traits_unwrap = MakerTraitsBuilder::new(env.clone())
        .unwrap_weth()
        .build();
...
fn test_maker_traits_builder_complex() {
    ...
    assert!(!MakerTraitsLib::has_extension(env.clone(), complex_traits.clone()));
    assert!(!MakerTraitsLib::is_expired(&env, complex_traits.clone()));
    assert_eq!(MakerTraitsLib::nonce_or_epoch(&env, complex_traits.clone()), 12345);
    assert_eq!(MakerTraitsLib::series(&env, complex_traits.clone()), 67890);
    assert!(!MakerTraitsLib::allow_partial_fills(&env, complex_traits.clone()));
    assert!(MakerTraitsLib::need_pre_interaction_call(&env, complex_traits.clone()));
    assert!(MakerTraitsLib::need_post_interaction_call(&env, complex_traits.clone()));
    assert!(MakerTraitsLib::allow_multiple_fills(&env, complex_traits.clone()));
    assert!(MakerTraitsLib::need_check_epoch_manager(&env, complex_traits.clone()));
    assert!(MakerTraitsLib::use_permit2(env.clone(), complex_traits.clone()));
    assert!(MakerTraitsLib::unwrap_weth(env.clone(), complex_traits.clone()));
}
...
fn test_maker_traits_builder_default() {
    ...
    // Verify all defaults
    assert!(!MakerTraitsLib::has_extension(env.clone(), default_traits.clone()));
    assert!(!MakerTraitsLib::is_expired(&env, default_traits.clone()));
    assert_eq!(MakerTraitsLib::nonce_or_epoch(&env, default_traits.clone()), 0);
    assert_eq!(MakerTraitsLib::series(&env, default_traits.clone()), 0);
    assert!(MakerTraitsLib::allow_partial_fills(&env, default_traits.clone()));
    assert!(!MakerTraitsLib::need_pre_interaction_call(&env, default_traits.clone()));
    assert!(!MakerTraitsLib::need_post_interaction_call(&env, default_traits.clone()));
    assert!(!MakerTraitsLib::allow_multiple_fills(&env, default_traits.clone()));
    assert!(!MakerTraitsLib::need_check_epoch_manager(&env, default_traits.clone()));
    assert!(!MakerTraitsLib::use_permit2(env.clone(), default_traits.clone()));
...
fn test_edge_cases_zero_values() {
    ...
    assert!(MakerTraitsLib::is_allowed_sender(&env, zero_traits.clone(), any_address));
...
fn test_edge_cases_max_values() {
    ...
    assert_eq!(MakerTraitsLib::nonce_or_epoch(&env, max_traits.clone()), max_40_bits);
    assert_eq!(MakerTraitsLib::series(&env, max_traits.clone()), max_40_bits);
...
fn test_bit_invalidator_logic_combinations() {
    ...
    let case2 = MakerTraitsBuilder::new(env.clone())
