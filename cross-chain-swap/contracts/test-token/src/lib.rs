...
use soroban_sdk::{
    contract, contractimpl, contracttype, symbol_short, Address, Env, Symbol, String, Map
};
...
impl TestToken {
    pub fn __constructor(env: Env, admin: Address) {
        ...
        env.storage().persistent().set(&(BALANCE, admin.clone()), &total_supply);
    }
    ...
    pub fn allowance(env: Env, from: Address, spender: Address) -> i128 {
        ...
        env.storage().persistent().get(&(ALLOWANCE, key)).unwrap_or(0)
    }
    ...
    pub fn approve(env: Env, from: Address, spender: Address, amount: i128, expiration_ledger: u32) {
        ...
        if expiration_ledger < env.ledger().sequence() {
            env.storage().persistent().extend_ttl(&(ALLOWANCE, AllowanceDataKey { from, spender }), 100, 100);
        }
    }
    ...
    pub fn transfer_from(env: Env, spender: Address, from: Address, to: Address, amount: i128) {
        ...
        env.storage().persistent().set(&(ALLOWANCE, allowance_key), &(allowance - amount));
        Self::transfer_impl(env, from, to, amount);
    }
    ...
    pub fn mint(env: Env, admin: Address, to: Address, amount: i128) {
        ...
        env.storage().persistent().set(&(BALANCE, to), &(balance + amount));
        env.storage().persistent().set(&TOTAL_SUPPLY, &(total_supply + amount));
    }
