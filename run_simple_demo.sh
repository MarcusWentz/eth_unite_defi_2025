...
# Simple 1inch Fusion+ Cross-Chain Swap Demo
...
check_stellar_network() {
    ...
    if curl -s http://localhost:8000/health >/dev/null 2>&1; then
        ...
        print_warning "Stellar network not running."
        print_status "Please start the Stellar network first:"
        print_status "1. Run: ./deploy_full_stack.sh"
        print_status "2. Or start manually with Docker"
        return 1
    fi
}
...
run_demo() {
    ...
    print_status "• Stellar smart contract integration"
...
show_results() {
    ...
    echo "  ✅ Stellar smart contract integration"
...
main() {
    ...
    if ! check_stellar_network; then
        print_error "Please start the Stellar network first"
        print_status "Run: ./deploy_full_stack.sh"
        exit 1
    fi
