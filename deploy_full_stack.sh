...
stellar keys generate \
  ...
  --network local \
  ${STELLAR_IDENTITY_NAME} > /dev/null
...
DA_WASM_PATH="./target/wasm32-unknown-unknown/release/${SOROBAN_DA_WASM_NAME}.wasm"
...
WASM_PATH="./target/wasm32-unknown-unknown/release/${SOROBAN_WASM_NAME}.wasm"
...
# Note: Contract invocation with complex UDT structs is not yet fully supported in Stellar CLI
# The contracts have been successfully deployed and are ready for use
success "Contracts deployed successfully! Contract IDs:"
success "  - Dutch Auction: ${DA_CONTRACT_ID}"
success "  - Order: ${CONTRACT_ID}"
success "Contracts are ready for interaction via other tools or direct RPC calls."
